...
import { Pencil, Trash2, Plus, Upload, LogOut, Lock } from "lucide-react";
...
const Admin = () => {
  ...
  const load = async () => {
    try {
      const { products } = await adminApi.list();
      setItems(products ?? []);
    } catch (e: any) {
            ...
            <div key={p.id} className="bg-card rounded-2xl shadow-soft p-3 flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <button
                  ...
                  className="h-8 w-8 rounded-lg border border-border flex items-center justify-center hover:bg-destructive hover:text-destructive-foreground"
                  aria-label="Excluir"
                >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, base_price: Number(e.target.value) || 0 })
                  }
                  className="mt-1"
                />
              </div>
