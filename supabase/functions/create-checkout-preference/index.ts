// Edge function: cria preferência de Checkout Pro no Mercado Pago (cartão, boleto, etc.)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const siteOrigin = req.headers.get("origin") || "https://criartpersonalizadosdl.lovable.app";
    const returnUrl = `${siteOrigin}/pagamento/${orderId}`;
    ...
    if (!mpRes.ok) {
      ...
      return json(
        { error: mpData?.message || "Erro ao criar checkout", details: mpData },
        500,
      );
    }
    ...
    if (!initPoint) {
      return json({ error: "Link de pagamento não retornado pelo Mercado Pago" }, 500);
    }
    ...
    return json({ init_point: initPoint, preference_id: mpData.id });
  } catch (e) {
    console.error(e);
